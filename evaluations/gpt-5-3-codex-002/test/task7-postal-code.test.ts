...
describe("getPostalCode", () => {
  ...
  it("returns None when the value is missing or blank", () => {
    expect(getPostalCode({})).toEqual({ type: "none" });
    expect(getPostalCode({ user: {} })).toEqual({ type: "none" });
    expect(getPostalCode({ user: { profile: {} } })).toEqual({ type: "none" });
  ...
  it("does not throw for nullish nested values from external JSON", () => {
    ...
    expect(getPostalCode({ user: { profile: { address: null } } })).toEqual({ type: "none" });
  });
});
